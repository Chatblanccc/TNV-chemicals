...
import type { PublishedApplication, PublishedArticle, PublishedCertificate, PublishedCompanyProfile, PublishedDownload, PublishedProduct, PublishedSiteContent } from "./published-content";
...
const certificateStatusLabel = (locale: Locale, certificate: { issuedDate: string; expiresDate?: string; validity: "current" | "expired" | "unspecified" }) => {
  const issued = locale === "zh" ? `签发：${certificate.issuedDate}` : `Issued ${certificate.issuedDate}`;
  if (certificate.validity === "current") return `${issued} · ${locale === "zh" ? `有效至：${certificate.expiresDate}` : `Valid through ${certificate.expiresDate}`}`;
  if (certificate.validity === "expired") return `${issued} · ${locale === "zh" ? `已于 ${certificate.expiresDate} 过期` : `Expired ${certificate.expiresDate}`}`;
  return `${issued} · ${locale === "zh" ? "未提供到期日" : "Expiry date not provided"}`;
};
...
function ProductDocuments({ product, locale, downloads }: { product: PublishedProduct; locale: Locale; downloads: PublishedDownload[] }) {
  const productDocuments = downloads.filter(document => document.productSlug === product.slug && document.locale === locale);
...
function ApplicationDetail({ item, locale, siteProducts }: { item: PublishedApplication; locale: Locale; siteProducts: PublishedProduct[] }) {
  const governedRelationship = item.verificationStatus === "verified";
  const related = governedRelationship
    ? siteProducts.filter(product => item.relatedProductSlugs?.includes(product.slug))
    : siteProducts.filter(product => product.category === "printing-inks").slice(0,3);
  const challenges = locale === "zh" && item.challengesZh?.length ? item.challengesZh : item.challenges;
  const relationshipCopy = governedRelationship
    ? { eyebrow: locale === "zh" ? "已核实产品关系" : "VERIFIED PRODUCT RELATIONSHIPS", heading: locale === "zh" ? "适用于此应用的已关联产品" : "Products linked to this application" }
    : { eyebrow: t(locale, "RELATED STARTING POINTS"), heading: t(locale, "Representative product families") };
  return <main><PageHero locale={locale} eyebrow="APPLICATION GUIDE" title={localizedValue(locale, item.name, item.nameZh)} intro={localizedValue(locale, item.intro, item.introZh)}/><section className="section product-content"><div><span className="eyebrow">{t(locale, "COMMON BUYER CHALLENGES")}</span><h2>{t(locale, "Define the production window first.")}</h2><ul className="benefit-list">{challenges.map(challenge => <li key={challenge}>{locale === "zh" && item.challengesZh?.length ? challenge : t(locale, challenge)}</li>)}</ul></div><div><span className="eyebrow">{t(locale, "QUALIFICATION INPUTS")}</span><h2>{t(locale, "Prepare these details")}</h2><table><tbody><tr><th>{t(locale, "Substrate")}</th><td>{t(locale, "Material, supplier and surface treatment")}</td></tr><tr><th>{t(locale, "Process")}</th><td>{t(locale, "Print method, speed and drying configuration")}</td></tr><tr><th>{t(locale, "End use")}</th><td>{t(locale, "Resistance, finishing and regulatory requirements")}</td></tr></tbody></table></div></section><section className="section"><div className="section-head"><div><span className="eyebrow">{relationshipCopy.eyebrow}</span><h2>{relationshipCopy.heading}</h2></div></div>{related.length ? <div className="product-grid">{related.map(product => <ProductCard key={product.slug} product={product} locale={locale}/>)}</div> : <div className="review-note"><b>{locale === "zh" ? "尚未发布已核实的产品关系" : "No verified product relationship is published yet"}</b><p>{locale === "zh" ? "请提交承印材料、工艺和性能要求，由技术团队完成选型。" : "Submit the substrate, process and performance target so the technical team can qualify a suitable family."}</p><Link className="text-link dark" href={localizedPath(locale, "/request-quote")}>{locale === "zh" ? "提交技术询盘" : "Submit a technical inquiry"} <Arrow/></Link></div>}</section><Rfq locale={locale} /></main>;
}
...
function CompanyProfile({ locale, published }: { locale: Locale; published: PublishedSiteContent }) {
  ...
  const certificateNames = published.certificates.filter(certificate => certificate.validity === "current").map(certificate => localizedValue(locale, certificate.name, certificate.nameZh));
  ...
  const rows: Array<[string, React.ReactNode]> = [
    ...
    [locale === "zh" ? "当前有效证书" : "Current certificates", certificateNames.length ? certificateNames.join(locale === "zh" ? "、" : ", ") : pending("certificates", "no current verified certificates published", "尚无当前有效的已验证证书")],
...
function StandardPage({ route, locale, productCode, initialRequirement, published }: { route: string; locale: Locale; productCode?: string; initialRequirement?: string; published: PublishedSiteContent }) {
  ...
  const currentCertificates = published.certificates.filter(certificate => certificate.validity === "current");
  ...
  if (route === "/technical-library" || route === "/certificates" || route === "/downloads") {
    type LibraryResource = (PublishedCertificate & { kind: string }) | (PublishedDownload & { kind: string });
    const localizedDownloads = published.downloads.filter(item => item.locale === locale);
    const resources: LibraryResource[] = route === "/certificates" ? published.certificates.map(item => ({ ...item, kind: item.type })) : route === "/downloads" ? localizedDownloads.map(item => ({ ...item, kind: item.type.toUpperCase() })) : [...localizedDownloads.map(item => ({ ...item, kind: item.type.toUpperCase() })), ...published.certificates.map(item => ({ ...item, kind: item.type }))];
    return <main><PageHero locale={locale} eyebrow={eyebrow} title={title} intro={intro}/>{resources.length ? <section className="section library-grid">{resources.map(item => {
      const certificate = "validity" in item ? item : null;
      const download = certificate ? null : item as PublishedDownload & { kind: string };
      const linkedProduct = download?.productSlug ? published.products.find(product => product.slug === download.productSlug) : null;
      const downloadStatus = download ? [localeRegistry[download.locale].nativeLabel, linkedProduct ? localizedValue(locale, linkedProduct.name, linkedProduct.nameZh) : (locale === "zh" ? "通用资料" : "General library")].join(" · ") : "";
      return <a key={`${item.kind}-${item.slug}`} className={certificate ? `certificate-resource certificate-${certificate.validity}` : "download-resource"} href={item.fileUrl} target="_blank" rel="noreferrer" data-analytics-download={item.slug}><span>{item.kind}</span>{certificate ? <small className="resource-status">{certificateStatusLabel(locale, certificate)}</small> : <small className="resource-status">{downloadStatus}</small>}<h2>{localizedValue(locale, item.name, item.nameZh)}</h2><p>{localizedValue(locale, item.description || "Verified company document", item.descriptionZh)}</p><b>{certificate?.validity === "expired" ? (locale === "zh" ? "打开历史文件" : "Open historical file") : (locale === "zh" ? "打开已验证文件" : "Open verified file")} <Arrow/></b></a>;
    })}</section> : <section className="section library-grid">{["Product catalog","Technical data sheets","Safety data sheets","Certificates","Testing reports","Packaging information"].map(x => <div key={x}><span>PDF</span><h2>{t(locale, x)}</h2><p><Placeholder locale={locale}>{t(locale, "company files pending")}</Placeholder></p><button disabled>{t(locale, "Not yet available")}</button></div>)}</section>}</main>;
  }
  return <main><PageHero locale={locale} eyebrow={eyebrow} title={title} intro={intro}/><section className="section editorial"><div><h2>{verifiedCompany ? (locale === "zh" ? "已核实企业信息" : "Verified company information") : t(locale, "Company review required")}</h2><p>{t(locale, "This page is intentionally structured without invented facts. Replace the review markers with verified company data, original photography, responsible personnel and dated evidence.")}</p></div><div className="fact-sheet"><p><span>{t(locale, "Legal company name")}</span>{verifiedCompany?.legalName ? localizedValue(locale, verifiedCompany.legalName, verifiedCompany.legalNameZh) : <Placeholder locale={locale}>{t(locale, company.legalName)}</Placeholder>}</p><p><span>{t(locale, "Factory location")}</span>{verifiedCompany?.address ? localizedValue(locale, verifiedCompany.address, verifiedCompany.addressZh) : <Placeholder locale={locale}>{t(locale, company.address)}</Placeholder>}</p><p><span>{locale === "zh" ? "当前有效证书" : "Current certifications"}</span>{currentCertificates.length ? currentCertificates.map(item => localizedValue(locale, item.name, item.nameZh)).join(locale === "zh" ? "、" : ", ") : <Placeholder locale={locale}>{locale === "zh" ? "当前有效的证书名称与日期" : "current verified certificate names and dates"}</Placeholder>}</p><p><span>{t(locale, "Production capability")}</span>{verifiedCompany?.manufacturingCapability ? localizedValue(locale, verifiedCompany.manufacturingCapability, verifiedCompany.manufacturingCapabilityZh) : <Placeholder locale={locale}>{t(locale, "verified lines and annual capacity")}</Placeholder>}</p></div></section><Rfq locale={locale} compact/></main>;
}
