...
function MediaImage({ item, className }: { item: SiteMedia; className?: string }) {
  const priority = item.usage.includes("hero");
...
function Header({ locale, route }: { locale: Locale; route: string }) {
  ...
  return <>
    <div className="review-bar"><span>{t(locale, "PRE-LAUNCH REVIEW SITE")}</span><span>{t(locale, "Unverified company facts are clearly marked")}</span></div>
    <header className="header">
      ...
      <div className="header-actions">
        ...
        <MobileNav items={items.map(({ href, label }) => ({ href, label }))} ctaHref={path("/request-quote")} ctaLabel={t(locale, "Request a quote")} />
      </div>
    </header>
  </>;
}
...
function Footer({ locale }: { locale: Locale }) {
  ...
  return <footer>
    ...
    <div className="footer-bottom"><span>© 2026 TNV Chemicals. {t(locale, "Pre-launch content.")}</span><span><Link href={path("/privacy-policy")}>{t(locale, "Privacy")}</Link> · <Link href={path("/terms")}>{t(locale, "Terms")}</Link></span></div>
  </footer>;
}
...
function Home({ locale }: { locale: Locale }) {
  ...
  return <main>
    <section className="hero">
      <div className="hero-copy"><span className="eyebrow">{t(locale, "PRINTING INKS · FUNCTIONAL MATERIALS · CUSTOM FORMULATION")}</span><h1>{t(locale, "Industrial chemistry,")}<br /><em>{t(locale, "built around your process.")}</em></h1><p>{t(locale, "Application-led ink and chemical solutions for packaging, paper and specialty print buyers—from initial sample to repeatable production.")}</p><div className="hero-actions"><Link className="button button-dark" href={path("/products")}>{t(locale, "Explore products")} <Arrow /></Link><Link className="text-link dark" href={path("/request-quote")}>{t(locale, "Discuss an application")} <Arrow /></Link></div><div className="hero-folio"><b>01</b><span></span><small>Material atlas</small></div></div>
      <div className="hero-atlas"><MediaImage item={media.heroInk} /><MediaImage item={media.heroPaper} /><MediaImage item={media.heroCorrugated} /><p>Ink. Substrate. Structure.<br />Aligned for performance.</p></div>
    </section>
    ...
    <section className="atlas-section evidence-section"><div className="section-intro"><span className="section-number">04</span><h2>Built on R&amp;D.<br />Delivered in manufacturing.</h2><p>{t(locale, "This page is intentionally structured without invented facts. Replace the review markers with verified company data, original photography, responsible personnel and dated evidence.")}</p><Link className="text-link dark" href={path("/about")}>{t(locale, "ABOUT TNV CHEMICALS")} <Arrow /></Link></div><div className="evidence-grid">{evidence.map(([image, label, title, href]) => <Link key={label} href={href}><MediaImage item={image} /><span>{label}</span><h3>{title}</h3><small><Placeholder locale={locale}>{t(locale, "verified workflow details")}</Placeholder></small></Link>)}</div></section>

    <section className="technical-index"><div><span className="section-number">05</span><h2>{t(locale, "TECHNICAL LIBRARY")}</h2><p>{t(locale, "Tools, guidance and resources for technical, R&D and procurement teams.")}</p><Link className="text-link dark" href={path("/technical-library")}>{t(locale, "Open technical library")} <Arrow /></Link></div><div className="technical-links"><Link href={path("/technical-library")}><b>{t(locale, "Technical Data Sheet")}</b><span>{t(locale, "Specification by product grade")}</span><Arrow /></Link><Link href={path("/technical-library")}><b>{t(locale, "Safety Data Sheet")}</b><span>{t(locale, "Market and language specific")}</span><Arrow /></Link><Link href={path("/insights")}><b>{t(locale, "TECHNICAL INSIGHTS")}</b><span>{t(locale, "Useful answers for industrial buyers.")}</span><Arrow /></Link></div></section>

    <section className="closing-cta"><h2>Let’s create chemistry<br /><em>that works for your process.</em></h2><div><p>{t(locale, "Share your substrate, print method, target performance and estimated volume. A technical sales contact can then qualify the right next step.")}</p><Link className="button" href={path("/request-quote")}>{t(locale, "Start a technical inquiry")}<Arrow /></Link></div></section>
  </main>;
}
