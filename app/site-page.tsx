...
import { applications, company } from "./site-data";
...
import type { PublishedArticle, PublishedProduct, PublishedSiteContent } from "./published-content";
...
function Home({ locale, siteProducts }: { locale: Locale; siteProducts: PublishedProduct[] }) {
  const path = (target: string) => localizedPath(locale, target);
...
function ProductDetail({ product, locale }: { product: PublishedProduct; locale: Locale }) {
  const path = (target: string) => localizedPath(locale, target);
  const image = product.category === "printing-inks" ? media.materialInks : product.category === "colorants" ? media.materialColorants : product.category === "additives" ? media.materialAdditives : media.materialCustom;
...
function ApplicationDetail({ item, locale, siteProducts }: { item: typeof applications[number]; locale: Locale; siteProducts: PublishedProduct[] }) {
...
function StandardPage({ route, locale, productCode, published }: { route: string; locale: Locale; productCode?: string; published: PublishedSiteContent }) {
  ...
  if (route === "/technical-library" || route === "/certificates" || route === "/downloads") {
    ...
    return <main><PageHero locale={locale} eyebrow={eyebrow} title={title} intro={intro}/>{resources.length ? <section className="section library-grid">{resources.map(item => <a key={`${item.kind}-${item.slug}`} href={item.fileUrl} target="_blank" rel="noreferrer"><span>{item.kind}</span><h2>{localizedValue(locale, item.name, item.nameZh)}</h2><p>{localizedValue(locale, item.description || "Verified company document", item.descriptionZh)}</p><b>{locale === "zh" ? "打开已验证文件" : "Open verified file"} <Arrow/></b></a>)}</section> : <section className="section library-grid">{["Product catalog","Technical data sheets","Safety data sheets","Certificates","Testing reports","Packaging information"].map(x => <div key={x}><span>PDF</span><h2>{t(locale, x)}</h2><p><Placeholder locale={locale}>{t(locale, "company files pending")}</Placeholder></p><button disabled>{t(locale, "Not yet available")}</button></div>)}</section>}</main>;
  }
...
function Insights({ slug, locale, routeBase, siteArticles, siteProducts }: { slug?: string; locale: Locale; routeBase: "/insights" | "/knowledge"; siteArticles: PublishedArticle[]; siteProducts: PublishedProduct[] }) {
...
function AdminNavigation({ locale, route }: { locale: Locale; route: string }) {
  const items = [["/admin", locale === "zh" ? "总览" : "Overview"], ["/admin/inquiries", locale === "zh" ? "询盘" : "Inquiries"], ["/admin/content", locale === "zh" ? "内容" : "Content"], ["/admin/seo", "SEO"], ["/admin/users", locale === "zh" ? "权限" : "Access"]];
...
function AdminHub({ locale }: { locale: Locale }) {
  const items = [["/admin/inquiries", locale === "zh" ? "询盘管理" : "Inquiry management", locale === "zh" ? "筛选线索并记录销售阶段。" : "Qualify leads and maintain the sales pipeline."], ["/admin/content", locale === "zh" ? "内容管理" : "Content management", locale === "zh" ? "维护产品、文章、证书与下载。" : "Maintain products, articles, certificates and downloads."], ["/admin/seo", "SEO management", locale === "zh" ? "维护页面级搜索信息。" : "Manage page-level search metadata."], ["/admin/users", locale === "zh" ? "权限管理" : "Access control", locale === "zh" ? "配置管理员角色与权限。" : "Configure admin roles and permissions."]];
...
function AdminRoutePage({ locale, route }: { locale: Locale; route: string }) {
  const copy: Record<string, [string, string, string]> = {
    ...
    "/admin/content": [locale === "zh" ? "内容管理" : "CONTENT MANAGEMENT", locale === "zh" ? "让产品与资料经过审核后再发布。" : "Publish product and technical content through review.", locale === "zh" ? "产品、文章、证书与下载各自保留验证状态、发布状态与操作记录。" : "Products, articles, certificates and downloads retain independent verification, publishing and audit states."],
  ...
  return <main className="admin-page"><section className="admin-heading"><span className="eyebrow">{eyebrow}</span><h1>{title}</h1><p>{intro}</p></section><AdminNavigation locale={locale} route={route}/>{route === "/admin" ? <AdminHub locale={locale}/> : route === "/admin/inquiries" ? <AdminInquiries locale={locale}/> : route === "/admin/content" ? <AdminContent locale={locale}/> : route === "/admin/seo" ? <AdminSeo locale={locale}/> : <AdminUsers locale={locale}/>}</main>;
}
...
export function SitePage({ route: inputRoute, content, searchParams = {} }: { route: string; content: PublishedSiteContent; searchParams?: Record<string, string | string[] | undefined> }) {
  ...
  let page: React.ReactNode = <StandardPage route={route} locale={locale} productCode={requestedProduct} published={content}/>;
  if (route === "/") page = <Home locale={locale} siteProducts={siteProducts}/>;
  ...
  else if (route === "/insights" || route === "/knowledge" || insightSlug) page = <Insights slug={insightSlug} locale={locale} routeBase={route.startsWith("/knowledge") ? "/knowledge" : "/insights"} siteArticles={siteArticles} siteProducts={siteProducts}/>;
