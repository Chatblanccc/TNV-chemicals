...
function Home({ locale }: { locale: Locale }) {
  ...
  return <main>
    <section className="hero">
      ...
      <div className="hero-atlas"><MediaImage item={media.heroInk} priority /><MediaImage item={media.heroPaper} /><MediaImage item={media.heroCorrugated} /><p>Ink. Substrate. Structure.<br />Aligned for performance.</p></div>
    </section>
...
export function SitePage({ route: inputRoute, searchParams = {} }: { route: string; searchParams?: Record<string, string | string[] | undefined> }) {
  ...
  const schema = route === "/"
    ...
    : product
      ? { "@context": "https://schema.org", "@type": "Product", "@id": pageUrl ? `${pageUrl}#product` : undefined, name: t(locale, product.name), description: t(locale, product.use), sku: product.code, category: t(locale, product.categoryName), image: baseUrl && productImage ? `${baseUrl}${productImage.src}` : undefined, url: pageUrl, inLanguage: language, brand: { "@type": "Brand", name: company.brand } }
      : article
        ? { "@context": "https://schema.org", "@type": "Article", headline: t(locale, article.title), description: t(locale, article.summary), url: pageUrl, mainEntityOfPage: pageUrl, inLanguage: language, publisher: organization }
