...
export const routePaths = ["/", "/search", "/assistant", "/company-profile", "/products", ...products.map(p => `/products/${p.category}/${p.slug}`), "/applications", ...applications.map(a => `/applications/${a.slug}`), "/custom-solutions", "/quality-compliance", "/technical-library", "/certificates", "/downloads", "/about", "/about/factory", "/about/research-development", "/about/quality-control", "/insights", ...articles.map(a => `/insights/${a.slug}`), "/knowledge", ...knowledgeCategories.map(category => `/knowledge/${category.slug}`), ...articles.map(a => `/knowledge/${a.slug}`), "/admin", "/admin/inquiries", "/admin/content", "/admin/seo", "/admin/analytics", "/admin/users", "/contact", "/request-quote", "/privacy-policy", "/terms"];
