...
export async function generateMetadata({ params }: { params: Promise<{slug: string[]}> }): Promise<Metadata> {
  ...
  const routeLabels: Record<string, string> = {
    ...
    "/about/quality-control": "QUALITY CONTROL", "/insights": "Insights", "/contact": "Contact",
    "/request-quote": "Request a quote", "/privacy-policy": "Privacy", "/terms": "Terms",
  };
  const label = route === "/" ? "TNV Chemicals" : product?.name || app?.name || article?.title || routeLabels[route] || "TNV Chemicals";
  ...
  return {
    ...
    openGraph: { title, description, type: article ? "article" : "website", locale: locale === "zh" ? "zh_CN" : "en_US", images: [{ url: "/og.png", width: 1536, height: 1024, alt: locale === "zh" ? "TNV Chemicals 工业油墨与应用解决方案" : "TNV Chemicals industrial ink and application solutions" }] },
