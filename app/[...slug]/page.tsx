...
import { applications, routePaths } from "../site-data";
...
function requireKnownRoute(route: string, content: PublishedSiteContent) {
  ...
  if (!routePaths.includes(route) && !dynamicProduct && !dynamicArticle && !dynamicCategory) notFound();
}
...
export async function generateMetadata({ params }: { params: Promise<{slug: string[]}> }): Promise<Metadata> {
  ...
  const routeLabels: Record<string, string> = {
    ...
    "/request-quote": "Request a quote", "/admin": "Administration", "/admin/inquiries": "Inquiry management", "/admin/content": "Content management", "/admin/seo": "SEO management", "/admin/users": "Access control", "/privacy-policy": "Privacy", "/terms": "Terms",
  };
  ...
  const localizedLabel = locale === "zh" ? (product?.nameZh || contentArticle?.titleZh || t(locale, label)) : label;
  ...
  const description = route === "/" ? (locale === "zh" ? "面向全球工业买家的应用导向型印刷油墨与化学解决方案。" : "Application-led printing ink and chemical solutions for international industrial buyers.") : locale === "zh" ? (product?.useZh || contentArticle?.summaryZh || t(locale, product?.use || app?.intro || contentArticle?.summary || fallbackDescription)) : (product?.use || app?.intro || contentArticle?.summary || fallbackDescription);
  ...
  return {
    ...
    openGraph: { title: resolvedTitle, description: resolvedDescription, type: contentArticle ? "article" : "website", locale: locale === "zh" ? "zh_CN" : "en_US", images: [{ url: "/og.png", width: 1536, height: 1024, alt: locale === "zh" ? "TNV Chemicals 工业油墨与应用解决方案" : "TNV Chemicals industrial ink and application solutions" }] },
    twitter: { card: "summary_large_image", title: resolvedTitle, description: resolvedDescription, images: ["/og.png"] },
