...
type Inquiry = { id: string; status: InquiryStatus; area: string; productCode: string | null; requirement: string; locale: string; notificationStatus: string; createdAt: number; updatedAt: number; email: string; company: string; country: string };
...
export function AdminInquiries({ locale }: { locale: "en" | "zh" }) {
  ...
  const copy = locale === "zh" ? { search: "搜索公司、邮箱、产品或需求", all: "全部状态", refresh: "刷新", empty: "当前筛选条件下没有询盘。", auth: "请先登录，再访问询盘后台。", forbidden: "当前账号尚未加入管理员白名单。", storage: "询盘数据库尚未准备完成。", generic: "无法读取询盘，请稍后重试。", signIn: "使用 ChatGPT 登录", created: "提交时间", notification: "通知", requirement: "生产需求" } : { search: "Search company, email, product or requirement", all: "All statuses", refresh: "Refresh", empty: "No inquiries match the current filters.", auth: "Sign in before opening the inquiry workspace.", forbidden: "This account is not configured in the admin allowlist.", storage: "The inquiry database is not ready yet.", generic: "Inquiries could not be loaded. Try again later.", signIn: "Sign in with ChatGPT", created: "Received", notification: "Notification", requirement: "Production requirement" };
  ...
  return <section className="admin-inquiries">
    ...
    {loading ? <div className="admin-loading" role="status">{locale === "zh" ? "正在读取询盘…" : "Loading inquiries…"}</div> : items.length === 0 ? <p className="admin-empty">{copy.empty}</p> : <div className="inquiry-list">{items.map(item => <article key={item.id}>
      ...
      <div className="inquiry-customer"><h2>{item.company}</h2><p>{item.email} · {item.country}</p></div>
